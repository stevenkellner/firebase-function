...
export function provideFirebaseFunctions(
    ...
    if (context instanceof FirebaseFunctionContext)
        return new AdminFirebaseFunction(context.Constructor, macKey).runnable(regions);
    if (context instanceof FirebaseRequestContext)
        return new AdminFirebaseRequest(context.Constructor, macKey).runnable(regions);
    if (context instanceof FirebaseScheduleContext)
        return new AdminFirebaseSchedule(context.Constructor, context.schedule, context.timezone).runnable(regions.length !== 0 ? regions[0] : null);
    return mapRecord(context, creator => provideFirebaseFunctions(creator, macKey, regions));
}
