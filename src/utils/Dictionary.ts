...
import { entries, keys, mapRecord, values } from './record';
...
export class DictionaryTypeBuilder<V, K, T> implements ITypeBuilder<Record<string, V>, Dictionary<K, T>> {
    ...
    public build(value: Record<string, V>, logger: ILogger): Dictionary<K, T> {
        return new Dictionary(mapRecord(value, value => this.builder.build(value, logger)));
    }
}
